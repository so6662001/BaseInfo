...
# 匹配样本环比
# --------------------------------------------------------------------------------------
def compute_sku_returns(panel: pd.DataFrame) -> pd.DataFrame:
    """计算每个 SKU 在相邻报价日之间的对数价比，只保留匹配样本。"""
    if panel.empty:
        return panel.assign(log_ret=pd.Series(dtype="float64"))

    out = panel.sort_values([COL_SKU, "date_rank"]).copy()
    g = out.groupby(COL_SKU, observed=True, sort=False)
    prev_price = g[COL_PRICE].shift(1)
    prev_rank = g["date_rank"].shift(1)
    prev_weight = g[COL_WEIGHT].shift(1)

    adjacent = (out["date_rank"] - prev_rank).eq(1) & prev_price.gt(0) & out[COL_PRICE].gt(0)
    with np.errstate(divide="ignore", invalid="ignore"):
        log_ret = np.log(out[COL_PRICE] / prev_price)
    out["log_ret"] = np.where(adjacent, log_ret, np.nan)
    out["pair_weight"] = np.where(adjacent, (out[COL_WEIGHT] + prev_weight) / 2.0, np.nan)
    return out
